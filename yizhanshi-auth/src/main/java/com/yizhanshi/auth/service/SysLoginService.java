...
import com.yizhanshi.common.core.enums.UserStatus;
...
import com.yizhanshi.common.core.utils.ip.IpUtils;
...
public class SysLoginService
{
     ...
     * 登录
     */
    public LoginUser login(String userStudentid,String  userName, String userPassword)
    {
        // 用户名或密码为空 错误
        if (StringUtils.isAnyBlank(userStudentid, userName,userPassword))
        {
            recordLogService.recordLogininfor(userName, Constants.LOGIN_FAIL, "用户/密码必须填写");
            throw new ServiceException("用户/密码必须填写");
        }
        // 密码如果不在指定范围内 错误
        if (userPassword.length() < UserConstants.PASSWORD_MIN_LENGTH
        ...
        // 用户名不在指定范围内 错误
        if (userStudentid.length() < UserConstants.USERNAME_MIN_LENGTH
                || userStudentid.length() > UserConstants.USERNAME_MAX_LENGTH)
        {
        ...
        // 查询用户信息
        R<LoginUser> userResult = remoteUserService.getUserInfo(userStudentid, SecurityConstants.INNER);
        ...
        if (UserStatus.DELETED.getCode().equals(user.getDelFlag()))
        {
            ...
            throw new ServiceException("对不起，您的账号：" + userStudentid + " 已被删除");
        }
        if (UserStatus.DISABLE.getCode().equals(user.getStatus()))
        {
            recordLogService.recordLogininfor(userName, Constants.LOGIN_FAIL, "用户已停用，请联系管理员");
            throw new ServiceException("对不起，您的账号：" + userStudentid + " 已停用");
        }
        sysPasswordService.validate(user, userPassword);
