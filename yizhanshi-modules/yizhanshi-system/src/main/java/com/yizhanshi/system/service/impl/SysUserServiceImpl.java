...
import com.yizhanshi.common.core.utils.SpringUtils;
import com.yizhanshi.common.core.utils.StringUtils;
import com.yizhanshi.common.core.utils.bean.BeanValidators;
...
import com.yizhanshi.system.domain.SysPost;
import com.yizhanshi.system.domain.SysUserPost;
import com.yizhanshi.system.domain.SysUserRole;
import com.yizhanshi.system.mapper.SysPostMapper;
import com.yizhanshi.system.mapper.SysRoleMapper;
import com.yizhanshi.system.mapper.SysUserMapper;
import com.yizhanshi.system.mapper.SysUserPostMapper;
...
public class SysUserServiceImpl implements ISysUserService
{
    ...
    {
        return userMapper.selectUserByUserStudentid(userStudentid);
    }
    ...
    {
        Long userId = StringUtils.isNull(user.getUserId()) ? -1L : user.getUserId();
        SysUser info = userMapper.checkUserStudentidUnique(user.getUserStudentid());
        if (StringUtils.isNotNull(info) && info.getUserId().longValue() != userId.longValue())
        {
            return UserConstants.NOT_UNIQUE;
        }
    ...
    {
        Long userId = StringUtils.isNull(user.getUserId()) ? -1L : user.getUserId();
        SysUser info = userMapper.checkPhoneUnique(user.getUserPhone());
        if (StringUtils.isNotNull(info) && info.getUserId().longValue() != userId.longValue())
        {
            return UserConstants.NOT_UNIQUE;
        }
    ...
    public String importUser(List<SysUser> userList, Boolean isUpdateSupport, String operName)
    {
        ...
        for (SysUser user : userList)
        {
            ...
            {
                // 验证是否存在这个用户
                SysUser u = userMapper.selectUserByUserStudentid(user.getUserStudentid());
                if (StringUtils.isNull(u))
                {
                    ...
                    successNum++;
                    successMsg.append("<br/>" + successNum + "、账号 " + user.getUserStudentid() + " 导入成功");
                }
                else if (isUpdateSupport)
                {
                    ...
                    successNum++;
                    successMsg.append("<br/>" + successNum + "、账号 " + user.getUserStudentid() + " 更新成功");
                }
                else
                {
                    failureNum++;
                    failureMsg.append("<br/>" + failureNum + "、账号 " + user.getUserStudentid() + " 已存在");
                }
            }
            catch (Exception e)
            {
                failureNum++;
                String msg = "<br/>" + failureNum + "、账号 " + user.getUserStudentid() + " 导入失败：";
